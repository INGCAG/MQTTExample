 ...
 * MQTT PUBLISH Frame
 */
open class MQTTFramePublish: MQTTFrame {
    var msgid: UInt16?
    var topic: String?
    var data: [UInt8]?
...
public protocol MQTTFrameBufferProtocol: class {
    func buffer(_ buffer: MQTTFrameBuffer, sendPublishFrame frame: MQTTFramePublish)
}

open class MQTTFrameBuffer: NSObject {
    
    open weak var delegate: MQTTFrameBufferProtocol?
    
//    let tim: SwiftyTimer
//    let tim = SwiftyTimer.new(after: 0, 0)
    
    
    // flow control
    fileprivate var silos = [MQTTFramePublish]()
    fileprivate var silosMaxNumber = 10
    fileprivate var buffer = [MQTTFramePublish]()
    ...
    open func add(_ frame: MQTTFramePublish) -> Bool {
    ...
    func send(_ frame: MQTTFramePublish) {
